...
export default function Cars() {
  const [cars, setCars] = React.useState(mockCars);
  ...
  const handleAdd = e => {
    e.preventDefault();
    setCars([...cars, { ...form, id: Math.random().toString(36).substring(2, 10) }]);
    setForm({ make: '', model: '', year: '', price: '' });
  };
  const handleDelete = id => setCars(cars.filter(c => c.id !== id));
  ...
  return (
    <Box sx={{
      ...
      <Typography variant="h4" color="#90caf9" gutterBottom fontWeight={700} letterSpacing={2} textShadow="0 2px 8px #0008">Gerenciar Carros</Typography>
      <Paper className="cars-card" sx={{ p: 3, mb: 4, width: '100%', maxWidth: 900 }}>
        <form onSubmit={handleAdd} style={{ width: '100%' }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ width: '100%' }}>
            <TextField fullWidth label="Marca" name="make" value={form.make} onChange={handleChange} required InputProps={{ style: { color: '#90caf9' } }} />
            <TextField fullWidth label="Modelo" name="model" value={form.model} onChange={handleChange} required InputProps={{ style: { color: '#90caf9' } }} />
            <TextField fullWidth label="Ano" name="year" value={form.year} onChange={handleChange} required type="number" InputProps={{ style: { color: '#90caf9' } }} />
      ...
      <TableContainer component={Paper} className="cars-card" sx={{ maxWidth: 900, width: '100%' }}>
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {cars.map(car => (
              <TableRow key={car.id}>
                ...
                <TableCell><Button color="error" onClick={() => handleDelete(car.id)}>Remover</Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
