...
const mockEmployees = [
  { id: 'E1', name: 'Carlos', position: 'Vendedor' },
  { id: 'E2', name: 'Ana', position: 'Gerente' },
];

export default function Employees() {
  const [employees, setEmployees] = React.useState(mockEmployees);
  const [form, setForm] = React.useState({ name: '', position: '' });
  ...
  const handleAdd = e => {
    e.preventDefault();
    setEmployees([...employees, { ...form, id: Math.random().toString(36).substring(2, 10) }]);
  ...
  return (
    <Box sx={{
      ...
      <Typography variant="h4" color="#90caf9" gutterBottom fontWeight={700} letterSpacing={2} textShadow="0 2px 8px #0008">Gerenciar Funcionários</Typography>
      <Paper className="employees-card" sx={{ p: 3, mb: 4, width: '100%', maxWidth: 900 }}>
        <form onSubmit={handleAdd} style={{ width: '100%' }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ width: '100%' }}>
            ...
            <TextField fullWidth label="Cargo" name="position" value={form.position} onChange={handleChange} required InputProps={{ style: { color: '#90caf9' } }} />
      ...
      <TableContainer component={Paper} className="employees-card" sx={{ maxWidth: 900, width: '100%' }}>
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {employees.map(employee => (
              <TableRow key={employee.id}>
                ...
                <TableCell><Button color="error" onClick={() => handleDelete(employee.id)}>Remover</Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
